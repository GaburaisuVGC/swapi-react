...
export default function StarshipsPage() {
 ...
 useEffect(() => {
   async function fetchData() {
     ...
     const response = await fetch(`http://localhost:8080/api/starships/${params.id.toString()}`);
    ...
    <section className="container-md">
      ...
      <Card style={{ width: "24rem", left: "33%", margin: "2%" }}>
              ...
              <ListGroup className="list-group-flush">
                ...
                <ListGroupItem>Hyperdrive rating : {form.hyperdrive_rating}</ListGroupItem>
                <ListGroupItem>MGLT : {form.MGLT}</ListGroupItem>
                <ListGroupItem>Starship class : {form.starship_class}</ListGroupItem>
                <ListGroupItem>URL : {form.url}</ListGroupItem>
              </ListGroup>
            </Card>
      </div>
      <div>
      <Button variant="primary" href="/starships">Back to the search list</Button>
      </div>
    </section>
 );
}